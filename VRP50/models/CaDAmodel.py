...
class VRP_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        if model_params['ffd'] == 'ffd':
            self.feed_forward = FeedForward(**model_params)
        elif model_params['ffd'] == 'siglu':   # siglu' 
            assert embedding_dim == 128
            self.feed_forward = ParallelGatedMLP()
        else:
            raise NotImplementedError
    ...
    def gate_and_attention_block(self, out_concat, context_embedding, cur_node_embedding, state_embedding):
        ...
        out2 = self.feed_forward(out1)
        mh_atten_out = self.add_n_normalization_2(out1, out2)
    ...
    def forward(self, td, cache, num_starts):
        ...
        # get last node 
        cur_node_embedding = gather_by_index(cache.node_embeddings, td["current_node"])  # bs, num_locs, embed 
        # get state original
        ...
        # post op
        logits = rearrange(score_scaled, "b s l -> (s b) l", s=num_starts)  # [(S*B), L]
        mask = rearrange(mask, "b s l -> (s b) l", s=num_starts)      # [(S*B), L]
